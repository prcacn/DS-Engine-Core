...
figma.ui.onmessage = function(msg) {
  if (msg.type === 'generate') { handleGenerate(msg.brief, msg.patternOverride); }
  if (msg.type === 'ping')     { handlePing(); }
...
function handlePing() {
  fetch(ENGINE_URL + '/health')
    ...
    .then(function(data) { figma.ui.postMessage({ type: 'ping-result', ok: true, data: data }); })
    .catch(function(e)  { figma.ui.postMessage({ type: 'ping-result', ok: false, error: e.message }); });
}
...
function paintScreen(engineResponse) {
  ...
  sorted.forEach(function(comp) {
    promise = promise.then(function(yPos) {
      return paintComponent(screenFrame, comp, yPos).then(function(height) { return yPos + height; });
    });
  });
...
function paintComponent(parent, comp, yPos) {
  ...
  return figma.getNodeByIdAsync(nodeId)
    .then(function(node) {
      if (!node) { createPlaceholder(parent, comp, yPos, height); return height; }
      var sourceNode = null;
      if (node.type === 'COMPONENT') { sourceNode = node; }
      else if (node.type === 'COMPONENT_SET') { sourceNode = node.defaultVariant || node.children[0]; }
      if (sourceNode && sourceNode.type === 'COMPONENT') {
        var instance = sourceNode.createInstance();
        instance.x = 0; instance.y = yPos;
        instance.resize(390, height);
...
figma.showUI(__html__, { width: 400, height: 760, title: 'DS Engine' });
