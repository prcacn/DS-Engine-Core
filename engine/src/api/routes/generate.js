...
// Devuelve: { 'card-item': 3, 'filter-bar': 2, ... }
...
function extractQuantities(brief) {
  ...
  // Mapeo de términos del brief a nombres de componente
  const TERM_TO_COMPONENT = {
    ...
    'botones primarios':'button-primary',
    ...
    'notificaciones':   'notification-banner',
    'alerta':           'notification-banner',
    'alertas':          'notification-banner',
    'banner':           'notification-banner',
    'tab':              'tab-bar',
    'tabs':             'tab-bar',
    'sección':          'list-header',
    'secciones':        'list-header',
    'badge':            'badge',
    'chip':             'badge',
    'chips':            'badge',
    'etiqueta':         'badge',
  ...
  // Patrón: (número|palabra) + (término de componente)
  // e.g. "tres card items", "3 filtros", "dos cards"
  const numPattern = '(\\d+|' + Object.keys(WORD_TO_NUM).join('|') + ')';
  const termPattern = Object.keys(TERM_TO_COMPONENT).map(function(t) {
  ...
  while ((match = regex.exec(b)) !== null) {
    ...
    // Buscar el componente que corresponde al término
    let component = null;
    for (const term of Object.keys(TERM_TO_COMPONENT)) {
      if (rawTerm.replace(/\s+/g, ' ').includes(term)) {
        component = TERM_TO_COMPONENT[term];
        break;
      }
    }
    if (component) {
      // Tomar el máximo si hay varios matches para el mismo componente
      quantities[component] = Math.max(quantities[component] || 0, num);
    }
  }
...
function resolveExclusivity(components, brief) {
  ...
  const hasCardItem   = components.some(function(c) { return c.component === 'card-item'; });
  const hasEmptyState = components.some(function(c) { return c.component === 'empty-state'; });
  if (hasCardItem && hasEmptyState) {
    if (hasExplicitEmpty) {
      return components.filter(function(c) { return c.component !== 'card-item'; });
    } else {
      return components.filter(function(c) { return c.component !== 'empty-state'; });
    }
  }
  return components;
}
...
function reorderComponents(components) {
  return components
    ...
    .map(function(c, i) { return Object.assign({}, c, { order: i + 1 }); });
}
  ...
  if (intentViolations && intentViolations.length > 0) {
    intentViolations.forEach(function(v) {
      violations.push({
        ...
        detail: v.detail,
        severity: v.severity,
        action: 'El engine ha ignorado esta parte del brief para mantener la gobernanza del DS'
      });
    });
  }
